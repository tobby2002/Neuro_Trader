...
def print_condition():
    ...
    if futures:
        fee_maker_maker = (fee_taker + fee_taker + fee_slippage) * leverage  # fee_maker buy:0.02%(0.0002) sell:0.02%(0.0002), sleepage:0.01%(0.0001)
        fee_maker_taker_slippage = (fee_taker + fee_maker + fee_slippage) * leverage  # fee_maker buy:0.02%(0.0002) sell:0.02%(0.0002), sleepage:0.01%(0.0001)
        print('(fee_maker_maker+fee_maker_taker_slippage)/2:%s%%' % (str(fee_maker_maker+fee_maker_taker_slippage)/2))
...
def backtest_trade45(df, wavepattern_up, symbol, trade_history):
    ...
    if futures:
        fee_maker_maker = (fee_maker + fee_maker + fee_slippage) * leverage  # fee_maker buy:0.02%(0.0002) sell:0.02%(0.0002), sleepage:0.01%(0.0001)
        fee_maker_taker_slippage = (fee_taker + fee_maker + fee_slippage) * leverage  # fee_maker buy:0.02%(0.0002) sell:0.02%(0.0002), sleepage:0.01%(0.0001)
    else:
    ...
    if closes:
        for i, c in enumerate(closes):
            ...
            # PT about long or SL about short
                ...
                asset_after = asset_total[-1] * (1 + pnl - (fee_maker_maker))
                asset_total.append(asset_after)
                trade_count.append(+1)
                fee_history.append(asset_total[-1]*(fee_maker_maker))
                pnl_history.append(asset_total[-1]*pnl)
                posi_end = dates[i]
                winrate = round((sum(trade_count)/len(trade_count))*100, 2)
                h1 = [len(trade_count), winrate, symbol, asset_after, round(pnl, 4), sum(pnl_history), sum(fee_history)]
                h2 = [str(w_start), str(w_end), (str(date), start), (str(posi_start), start), (str(posi_end), high + h_fibo_value)]
                h = [h1, h2, asset_total, trade_count, fee_history, pnl_history, wavepattern_up]
            ...
            # SL about long or PT about short
                ...
                asset_after = asset_total[-1] * (1 + pnl - (fee_maker_taker_slippage))
                asset_total.append(asset_after)
                trade_count.append(0)
                fee_history.append(asset_total[-1]*(fee_maker_taker_slippage))
                pnl_history.append(asset_total[-1]*pnl)
                posi_end = dates[i]
                winrate = round((sum(trade_count)/len(trade_count))*100, 2)
                h1 = [len(trade_count), winrate, symbol, asset_after, round(pnl, 4), sum(pnl_history), sum(fee_history)]
                        ...
                        if wavepattern_up.check_rule(rule):
                            if wavepattern_up in wavepatterns_up:
                                ...
                                if not check_has_same_wavepattern(wavepattern_up_l, wavepattern_up):
                                    ...
                                    trade_history = backtest_trade45(df_all, wavepattern_up, symbol, trade_history)
...
def round_trip(i):
    ...
    print(i, ' | ', trade_history[2][-1], len(trade_count), ' | ', asset_total[-1])
    return round_trip
...
if __name__ == '__main__':
    ...
    if round_trip_flg:
        ...
        for i in range(period_days_ago, period_days_ago_till, -1 * period_interval):
            asset_total_pre = [trade_history[2][-1]]
            single(symbols, i, trade_history)
            print(i, ' now asset: ', trade_history[2][-1], ' | ', len(trade_count), ' | pre seed: ', asset_total_pre[-1])

        print('============ %s stat.==========' % str(i))
        winrate_l = list(map(lambda i: 1 if i > 0 else 0, pnl_history))
        meanaverage = round((sum(asset_total)/len(asset_total)), 2)
