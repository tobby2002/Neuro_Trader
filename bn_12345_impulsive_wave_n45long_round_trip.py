...
period_round_trip = config['default']['period_round_trip']
...
def print_condition():
    ...
    # fee_o = (fee_maker + fee_maker + fee_slippage) * leverage  # fee_maker buy:0.02%(0.0002) sell:0.02%(0.0002), sleepage:0.01%(0.0001)
    ...
    print('period_round_trip: %s' % period_round_trip)
...
def loopsymbol(symbol, i, trade_history):
    ...
    try:
        ...
        # timeframe = 1  # 1, 5, 15
        timeunit = 'm'
        timeframe_str = str(timeframe) + timeunit
        ...
        df, start_date, end_date = get_historical_ohlc_data_start_end(symbol, start_str=start_str,
                                                                          ...
                                                                          interval=timeframe_str, futures=futures)
        df_all = df
...
def single(symbols, i, trade_history):
...
def round_trip(i):
    ...
    # h1 = [len(trade_count), winrate, symbol, asset_after, round(pnl, 2), sum(pnl_history), sum(fee_history)]
    # h2 = [str(w_start), str(w_end), (str(date), start), (str(posi_start), start),
    #       (str(posi_end), high + h_fibo_value)]
    trade_history = [None, None, asset_total, trade_count, fee_history, pnl_history, None]

    symbols = random.sample(symbols_usdt_yes, len(symbols_usdt_yes))
    symbols = symbols[:10]
    # print(symbols)
    single(symbols, i, trade_history)
    # round_trip.append((i, trade_history))
    round_trip.append(trade_history[2][-1])
    print(i, ' | ', trade_history[2][-1], len(trade_count))
    # print('============%s round end ===================================================' % i)
    return round_trip
if __name__ == '__main__':
    ...
    for i in range(period_round_trip):
        try:
        ...
        winrate_l = list(map(lambda i: 1 if i[0] > 1000 else 0, rt))
